...
module.exports = (sequelize, DataTypes) => {
     ...
     * @param  {string} UserTokens.username
     * @param  {string} UserTokens.sender_id
    ...
    /**
     * Returns if refresh_token has expired or not.
    ...
    /**
     * Returns if access_token has expired or not.
     * (Doesn't send any HTTP request, just checks in database last refresh time)
     * @return {boolean}
     */
    Model.prototype.hasAccessTokenExpired = function() {
        ...
        if (timeDifference > 3600) {
            console.log(`Access token of ${sender_id} expired since ${timeDifference}s`);
            return true;
        } else {
            return false;
        }
